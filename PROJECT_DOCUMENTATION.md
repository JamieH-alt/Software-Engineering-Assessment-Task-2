...
**-- Analyze the impact these updates could have on the user experience.** \
These changes would have a large-scale impact on the user experience, as they focus not just on the user, but also on the backend and modularity of the system for furthering development beyond just a single update. The Audio system would have the biggest impact out of the proposed additions/modifications for a next update, as Audio brings a whole other sense into the immersion and experience of a game. It's said that humans need 3 sensical stimulants to have full immersion -- to think somethings real -- and including audio with the game could more than double the sense of immersion players feel whilst playing the game. Alongside this the spell system would allow players to experience the fantastical reality of the world of Elednor and Dungeons and Critters in a way that could not be done before. These spells alongside better dungeons would completely overhaul the player experience when it comes to fighting -- something the game places a big focus on at the moment -- An update with these features would lead to players being unable to imagine themselves playing and enjoying the game before these features were added, leading to a giant impact on the user experience and perception of the games freshness.


**2. Evaluate the system in terms of how well it meets the requirements and specifications**\
-> **Requirements already covered in Review questions** <- \
The system in its final state meets the concepts behind the specifications to an extremely high level, however due to reality, the system does not always accurately represent the specifications. However the system **does** allow a user to access multiple saves, easily navigate between main uis, easily and quickly access inventory and stats, easily access a help page, system accepts UI related inputs and text based inputs, text inputs do accept all basic unicode characters (besides save names due to windows, mac and unix specifcations), the system outputs the players location, health, stats and other basic details, it also outputs players actions and details within the Terminal, it allows the user to interact with a TUI and an internal CLI. However the system does not in its current state access the DnD 5e's API -- this was modified to just store the details that WERE pulled from the DnD 5e API in a file (items.py) as it just made more sense in reality. The program also does not tell the user specifically what is wrong with their answers, however at all times it does provide enough details to allow the user (assuming an intelligent user) to make their own inferences and assumptions about what is allowed / incorrect within their inputs. The system does also not provide a basic crash log however the Textual library does automatically output the reasons for a crash.
The program and its systems do fit non-functional specifications perfectly, with the only inaccuracies being where the system out-performs the specifications. For example bigger actions do not require a longer wait time, and everything happens within less than a second. The program does use caching and hard coding efficiently, it also does not have ANY data coruption, and invalid data sources are a non-issue due to the use of caching and the removal of the DnD 5e API from the program's specifications.


**3. Evaluate your processes in terms of project management.** \
Throughout the 4 sprints involved in creating this program, I've had various ups and downs related to time management. Firstly once the project was given to us I rushed to do sprint 1 in an attempt to allow myself some breathing room throughout my extended leave to Japan. However I found that this was not enough to make up for my leave, as such - Japan was my first time management hurdle throughout this program however it was not my last. Outside of time management I found that starting the work on my projects practical portion before the theoretical portion was a management blunder, as it allowed my project to fall out of structure in some elements. Throughout the project I constantly found myself having to cut scope, matching the DND 5e API would be almost impossible for a solo developer due to the complex nature of DND and I should have done more research into the project's specifications in this aspect. With all this said I still managed to keep the project on course to fit specifications to an apt-level. The program does perform everything it set out to do, however this was because of a more broad definition of the project than expected. It's these scope cuts, time management errors, and structural issues that would lead me to evaluate my own project management skills as sub-par, however in lieu of these problems and strategical errors, I managed to keep the project from ending unfinished, and I met deadlines and specifications set out throughout the first sprint -- like the Gantt chart, or the Requirement Definitions -- to a level that would be difficult to achieve under normal circumstances, yet alone the circumstances I created for myself. It's due to this success in spite of adversary that I would evaluate my processes in project management to be performing at a high-degree with evident room for improvement.
...
|---|---|---|
| Very fun UI, Looks very good and interesting combat, Map is very good and I like the way it is, The text displayed really immerses the player fery well into the game fun once you get a hang of it I can change the THEME which is very good | A little hard to understand what to do at the start and game loop is a little boring | More adventures and a story, A story please |


I think from Ario's feedback -- whilst it didn't necesarilly focus on how ti meets non-function and functional specifications -- is very valuable to understand where I could have improved my program, and where my program's key strengths lie. Here Ario Illustrates that my program does need a bit more help, and this is true as at the time of the test the help.md file was not finished. And I also think that my game could do with more purpose; currently it feels like the player is just moving around aimlessly and killing zombies with no reason.

### My Sister Lily
| Plus | Minus | Interests |
| --- | --- | --- |
| Comprehensive UI Design, Keyboard Navigability, Saves work, Detailed displays, Intuitive instructions in command line | Spells would be fun, Limited interactions, Basic movement | The TUI is interesting, ASCII Art |

Personally I think that my project does have a lot going for it, and Lily's feedback reflects this mentioning the UI system, and it's navigability, as long as the intuitivity of the program. However she also pointed out soem areas for improvement. As I previously stated spells would bring a new fanatical element. I also agree that the basic movement and limited interactions can make the game feel very repetitive. She also made it clear that the design choise of using the TUI and the ASCII art was a good design choice (She works as a free-lance designer).
